...
export default Vue.defineComponent({
  ...
  setup() {
    ...
    const sortColumn = Vue.ref<
      'type' | 'queries' | 'price' | 'size' | 'millis' | 'name' | 'errors'
    >('name');
  ...
  methods: {
    clickedHeader(
      name: 'type' | 'queries' | 'price' | 'size' | 'millis' | 'name' | 'errors',
    ): void {
    ...
    updateList() {
      ...
      this.items = statsToUse.sort((a, b) => {
        if (sortColumn === 'type') {
          ...
          if (statsOrder !== 0n) return multiplier * Number(statsOrder);
