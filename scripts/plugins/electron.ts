  ...
  return [
    ...
    },
    {
      ...
      configResolved(config): void {
        ...
        const resolvedOutputs = resolveBuildOutputs(rollupOptions.output as any, libOptions);
    ...
    },
    {
      ...
      configResolved(config): void {
        if (config.base !== './' && config.base !== '/') {
          config.logger.warn(
            colors.yellow(
              '(!) Should not set "base" option for the electron vite renderer config.',
            ),
          );
        }
        ...
        if (!rollupOptions.input) {
          config.logger.warn(
            colors.yellow(
              `index.html file is not found in ${colors.dim('/src/renderer')} directory.`,
            ),
          );
          throw new Error(
            'build.rollupOptions.input option is required in the electron vite renderer config.',
          );
        }
      },
    },
