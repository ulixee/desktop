...
import { isFilePathESM, isObject } from './utils';
  ...
  if (configFile !== false) {
    ...
    if (loadResult) {
      ...
      if (loadResult.config.preload) {
        const preloadViteConfig: ViteConfig = mergeConfig(
      ...
      if (loadResult.config.renderer) {
        const rendererViteConfig: ViteConfig = mergeConfig(
...
export async function loadConfigFromFile(
  ...
  try {
    ...
    const userConfig = await loadConfigFormBundledFile(
      configRoot,
      resolvedPath,
      bundled.code,
      isESM,
    ...
    if (config.main) {
      ...
      mainConfig = await (typeof mainViteConfig === 'function'
        ? mainViteConfig(configEnv)
        : mainViteConfig);
...
async function loadConfigFormBundledFile(
  ...
  if (isESM) {
    ...
    require.extensions[loaderExt] = (module: NodeModule, filename: string): void => {
